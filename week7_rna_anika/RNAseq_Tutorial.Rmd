...
```{r}
rna_clinical <- read.csv(file = "/Users/anikakapai/Desktop/qbio_490_anika/week7_rna_anika/rna_clinical.csv", TRUE)
rna_counts <-read.csv(file = "/Users/anikakapai/Desktop/qbio_490_anika/week7_rna_anika/brca_rna_count_data.csv", TRUE)
rna_genes <-read.csv(file = "/Users/anikakapai/Desktop/qbio_490_anika/week7_rna_anika/brca_rna_gene_data.csv", TRUE)
```
...
summary(rna_clinical$age_category)
...
```{r}
# write code here
norm_mask <- ifelse(rna_clinical$definition == "Solid Tissue Normal", FALSE, TRUE)
```
...
```{r}
# write code here
rna_clinical <- rna_clinical[norm_mask,]
rna_counts <- rna_counts[,norm_mask]
```
...
five_yr_death <- ifelse(rna_clinical$days_to_death == "NA", NA, ifelse(rna_clinical$days_to_death >= 1826.25, TRUE, FALSE))

# use boolean data from days_to_last_followup when five_yr_death is NA

five_yr_death_and_followup <- ifelse(is.na(five_yr_death), ifelse(rna_clinical$days_to_last_follow_up >= 1826.25, TRUE, FALSE), five_yr_death)
...
BRCA1_mask <- ifelse(rna_genes$gene_name == "BRCA1", TRUE, FALSE)
```
...
The incidence of mutations for TP53 and BRCA1 seem to coincide, as shown by the high density of data points. I think that this means that the genes usually mutate together. This could be important in determining risk factors for BC.
----
...
```{r}
geneAB_counts <- data.frame(TP53_counts, BRCA1_counts) # create data frame with geneA_counts and geneB_counts
...
The dark horizontal bar is the median of the data. The box is the interquartile range. The BRCA1 data seems to lie far outside implying there is a greater number of mutations than TP53. 
----
...
There were fewer patients who survived longer than five years and had high mutation rates of TP53. These patients however had higher mutation rates (greater range) than those who did not survive five years. This makes sense upon research because survival is shown to decrease with greater TP53 mutations.
----
...
geneABCD_counts <- data.frame(MKI67_counts, PTGS2_counts, TP53_counts, BRCA1_counts)
colnames(geneABCD_counts) <- c("MKI67", "PTGS2", "TP53", "BRCA1")
...
jpeg("/Users/anikakapai/Desktop/test.jpeg")
...
```{r}
# save files here
write.csv(rna_counts, "/Users/anikakapai/Desktop/qbio_490_anika/week7_rna_anika/brca_rna_count_data.csv", row.names = TRUE)
write.csv(rna_genes, "/Users/anikakapai/Desktop/qbio_490_anika/week4_clinical_anika/analysis_data/brca_rna_gene_data.csv", row.names = TRUE)
write.csv(rna_clinical, "/Users/anikakapai/Desktop/qbio_490_anika/week4_clinical_anika/analysis_data/rna_clinical.csv", row.names = TRUE)
```
