...
```{r}
num_na_station <- sum(na_boolean_mask)
num_na_station
```
...
```{r}
nrow(attenu_cleaned)
nrow(attenu_cleaned) == nrow(attenu) - num_na_station
```
...
Choose any two columns from the iris data set. Create a scatterplot that compares these columns, color coded based on the values relative to the column means. Follow these steps to create your plot:
  ...
  2. Use a series of nested ifelse() statements to create a vector called colors. (HINT: In a nested ifelse(), the "else" condition is to run another ifelse(), and so on. Make sure to close the nest with the correct number of parentheses!) This vector should hold four colors. Assign a color (ex: "blue", "magenta", etc.) to each of the following scenarios:
    ...
    c. x is less than the x column mean and y is less than the y column mean = purple
    d. x is less than the x column mean but y is greater than the y column mean = gold
...
```{r}
# write code here
mean_petal_wid = mean(iris$Petal.Width) #x mean
mean_petal_len = mean(iris$Petal.Length) #y mean

colors = c()

colors <- ifelse(iris$Petal.Width > mean_petal_wid & iris$Petal.Length > mean_petal_len, colors <- "blue", 
       ifelse(iris$Petal.Width > mean_petal_wid & iris$Petal.Length < mean_petal_len, colors <- "red", 
              ifelse(iris$Petal.Width < mean_petal_wid & iris$Petal.Length < mean_petal_len, colors <- "purple",
                     ifelse(iris$Petal.Width < mean_petal_wid & iris$Petal.Length > mean_petal_len, colors <- "gold",
                            colors <- "no color"
                     )
              )
       )
...
print(cor.test(iris$Petal.Width, iris$Petal.Length))

plot(iris$Petal.Width, iris$Petal.Length, col = colors)

abline(h = mean_petal_len, v = mean_petal_wid)
```
...
```{r}
# write code here
write.csv(attenu_cleaned, "/Users/anikakapai/Desktop/attenu_cleaned.csv")
```
...
```{r}
# write code here
attenu_read_in <- read.csv("/Users/anikakapai/Desktop/attenu_cleaned.csv", header = TRUE)
```
...
```{r}
# write code here
pdf("/Users/anikakapai/Desktop/petal_hist.pdf")
hist(iris$Petal.Length)
dev.off()
```
